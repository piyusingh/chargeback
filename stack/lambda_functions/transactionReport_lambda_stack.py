...
#TransactionReport_lambda_stack
class TransactionReportLambdaStack(cdk.Stack):
    def __init__(self, scope: Construct, construct_id: str,common_stack,raw_config: EnvSpecific, **kwargs):
        ...
        ## Creation of TransactionReport Lambda
        self.transactionReport = lambda_.Function(self, "Chargeback_transactionreport",
                                        code=lambda_.Code.from_asset('lambdas/transactionReport'),
                                        function_name='Chargeback-TransactionReport',
                                        handler='Chargeback_TransactionReport.lambda_handler',
                                        vpc=self.Vpc,
                                        security_groups=[self.Security_Group],
                                        tracing=lambda_.Tracing.ACTIVE,
                                        runtime=lambda_.Runtime.PYTHON_3_7,
                                        timeout=cdk.Duration.seconds(raw_config.lambda_env_vars["lambda_timeouts"]),
                                        environment=raw_config.lambda_env_vars["transactionReport_lambda_vars"],
