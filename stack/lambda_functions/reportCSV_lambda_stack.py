...
#reportCSV_lambda_stack
class ReportCSVLambdaStack(cdk.Stack):
    def __init__(self, scope: Construct, construct_id: str,common_stack,raw_config: EnvSpecific, **kwargs):
        ...
        ## Creation of ReportCSV Lambda
        self.reportCSV = lambda_.Function(self, "Chargeback_reportcsv",
                                        code=lambda_.Code.from_asset('lambdas/reportCSV'),
                                        function_name='Chargeback-ReportCSV',
                                        handler='Chargeback_ReportCSV.lambda_handler',
                                        vpc=self.Vpc,
                                        security_groups=[self.Security_Group],
                                        tracing=lambda_.Tracing.ACTIVE,
                                        runtime=lambda_.Runtime.PYTHON_3_7,
                                        timeout=cdk.Duration.seconds(raw_config.lambda_env_vars["lambda_timeouts"]),
                                        environment=raw_config.lambda_env_vars["reportCSV_lambda_vars"],
