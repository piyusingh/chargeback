...
#AggregateReport_lambda_stack
class AggregateReportLambdaStack(cdk.Stack):
    def __init__(self, scope: Construct, construct_id: str,common_stack,raw_config: EnvSpecific, **kwargs):
        ...
        self.aggregateReport = lambda_.Function(self, "Chargeback_aggregatereport",
                                        code=lambda_.Code.from_asset('lambdas/aggregateReport'),
                                        function_name='Chargeback-AggregateReport',
                                        handler='Chargeback_AggregateReport.lambda_handler',
                                        vpc=self.Vpc,
                                        security_groups=[self.Security_Group],
                                        tracing=lambda_.Tracing.ACTIVE,
                                        runtime=lambda_.Runtime.PYTHON_3_7,
                                        timeout=cdk.Duration.seconds(raw_config.lambda_env_vars["lambda_timeouts"]),
                                        environment=raw_config.lambda_env_vars["aggregateReport_lambda_vars"],
